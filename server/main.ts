...
const refinerEndpoint = refinerAPI;
...
app.get("/smallcard/:id", async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const data = await fetchRefinerData(
        refinerEndpoint,
        id,
        requiresFullData,
      );
  ...
  try {
    ...
    try {
      ...
      const requiresFullData = [
        req.query.wantBannerImage,
        req.query.wantAccentColor,
      ].includes("true");
      const data = await fetchRefinerData(
        refinerEndpoint,
        id,
